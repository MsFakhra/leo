...
var to_support = [];
var negative = ['angry', 'disgust', 'anger','negative'];
var neutral = ['neutral', 'calm', 'analytical', 'tentative','neutral'];
var positive = ['happy', 'surprised', 'joy', 'confident','positive'];
var other = ['sad', 'fearful', 'fear', 'sadness','neutral'];
...
console.log('read_or_listen')
  ...
  // } else {
    var key=0
     ...
     /*$('#typefield').keydown(function(e){
        if(e.keyCode === 13){
           console.log('Enter: read_or_listen')
...
function both2(result){
  ...
  speech = result[0];
    ...
    text = "First tell me, Do you feel happy mostly or you feel upset from the things around you?";
    save_text=1;
    addLeo();
    speakVoice(text);
...
function startfromhere(_callback)
{
...
function compare(arr1){
  var neg_dict = {'angry': 0, 'disgust': 0, 'anger': 0, 'sad': 0, "fearful": 0, 'fear': 0, 'sadness': 0, 'negative':0}
  var pos_dict = {'neutral': 0, 'calm': 0, 'analytical': 0, 'tentative': 0, 'happy': 0, 'surprised': 0, 'joy': 0, 'positive':0}
...
function finalFeedback2(){
  text= "Hey did I tell you my favorite flower. It is Sunflower!\nI wanted to ask, do you feel better now? We will talk tomorrow.";
  save_text=1;
  addLeo();
  speakVoice(text);
...
function lasthing(){
// text= "Hope you have a nice day. Bye";
//  save_text=1;
//  addLeo();
//  speakVoice(text);
...
function bye(){
  ...
  $.ajax({
    ...
    success: function(response) {
      console.log('##############RESPONSE: ',emotionList)
      //(response);
    }
  }).done(function(data) {
...
console.log('createsentence')
  text= "I think we may talk later, You can say me bye now";
  save_text=1;
  addLeo();
  speakVoice(text);
  var url = "casual";
